...
def preprocess_seeds(seeds_list: List[str], stemmer) -> List[str]:
    ...
    seeds_stems = [list(map(lambda token: stemmer.stem(token), list_of_tokens)) for list_of_tokens in seeds_tokens]
    joined_seeds_stems = [" ".join(seed) for seed in seeds_stems]

    return joined_seeds_stems 


def find_most_freq_substring(list_of_substrings: List[List[str, str, int]]) -> List[str, str, int]:
