...
while len(heads_and_entities) > 0:
    ...
    phrases_with_seed = [(phrase, freq) for phrase, tag, freq in heads_and_entities if any(lemmatize(seed, nlp=nlp) in lemmatize(phrase, nlp=nlp) for seed in set_of_seeds)]
    ...
    heads_and_entities = [entity for entity in heads_and_entities if entity[0] != most_freq_substr]
