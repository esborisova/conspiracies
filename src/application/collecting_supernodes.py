...
stems = [list(map(lambda token: stemmer.stem(token), list_of_tokens)) for list_of_tokens in tokens]
...
while len(sorted_current_list)>0:
    ...
    substrings = [entity for seed in seeds_stems for entity in sorted_current_list if seed in entity[3]]
